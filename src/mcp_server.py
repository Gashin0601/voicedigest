"""
momo-accessibility MCP server — 全盲ユーザーが Slack で一番困る「画像」と「リンク」を
言葉にするためのツールを公開する MCP サーバー。

トランスポート = stdio（改行区切り JSON-RPC 2.0）。標準ライブラリのみ。
公開ツール:
  - describe_image(image_url, context?)  画像を OpenAI vision(gpt-4o-mini)で全盲向けに説明
  - read_link(url)                       リンク先を取得して音声向けに要約

環境変数:
  OPENAI_API_KEY   必須（無ければ sk- で始まる GEMINI_API_KEY を流用）
  OPENAI_MODEL     省略時 gpt-4o-mini（ビジョン対応）
...
# LLM = OpenAI（gpt-4o-mini はビジョン＋テキスト対応）。キーは OPENAI_API_KEY、
...
def _fetch_bytes(url):
    headers = {"User-Agent": "momo-mcp/1.0"}
...
def handle(msg):
    ...
    if method == "initialize":
        _result(req_id, {
            ...
            "capabilities": {"tools": {}},
            "serverInfo": {"name": "momo-accessibility", "version": "1.0.0"},
        })
